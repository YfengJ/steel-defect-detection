...
- Fix a reproducible training, validation, inference, conversion, or GUI problem.
...
- Improve actionable error messages in the desktop GUI.
...
```bash
python -m ruff check \
  train.py predict.py val.py video_predict.py translate.py ui.py setup.py \
  path_validation.py process_runner.py scripts tests --ignore F403,F405
...
Keep each pull request focused, describe verification evidence, and update user
